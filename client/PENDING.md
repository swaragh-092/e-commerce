...
## 🔴 Critical (Blockers — App is non-functional without these)

### 1. Cart Service & State — ENTIRELY MISSING
- [ ] Create `src/services/cartService.js`
  - `GET /cart` — fetch cart (optionalAuth)
  - `POST /cart/items` — `{ productId, variantId?, quantity }`
  - `PUT /cart/items/:id` — `{ quantity }`
  - `DELETE /cart/items/:id`
  - `DELETE /cart` — clear cart
  - `POST /cart/merge` — `{ sessionId }` — merge guest cart on login
- [ ] Create `src/context/CartContext.jsx`
  - Global state: `cart`, `cartCount`, `loading`
  - Methods: `addItem()`, `removeItem()`, `updateItem()`, `clearCart()`, `fetchCart()`
  - Persist `sessionId` in `localStorage` for guest carts
