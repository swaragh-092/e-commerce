...
const StorefrontOrderInvoicePage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    userService.getMyOrderById(id)
      ...
      .catch((err) => {
        setError(err?.response?.data?.error?.message || 'Failed to load order for invoice.');
      })
  ...
  return (
    ...
    >
      ...
      >
        <Button startIcon={<ArrowBackIcon />} onClick={() => navigate(`/account/orders/${id}`)}>
          Back to Order
        </Button>
...
export default StorefrontOrderInvoicePage;
